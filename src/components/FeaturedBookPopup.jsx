...
const FeaturedBookPopup = ({ isOpen, onClose, book, onBookNow }) => {
  ...
  return (
    ...
    >
      <div
        className="popup-typography flex max-h-[90vh] w-[min(58.75rem,96vw)] flex-col gap-2.5 overflow-hidden rounded-[0.625rem] border border-[#d1d1dd] bg-white px-[1.375rem] pb-7 pt-3 shadow-[0_0.75rem_1.75rem_rgba(10,10,35,0.18)] max-[43.75rem]:gap-1.5 max-[43.75rem]:p-[0.875rem] max-[43.75rem]:pb-5 dark:border-[#242732] dark:bg-[#14161b] dark:shadow-[0_0.75rem_1.75rem_rgba(0,0,0,0.45)]"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <h3
          ...
          className="mt-2 font-[family-name:var(--body-font)] text-2xl font-extrabold uppercase text-[#0a0b2b] max-[43.75rem]:text-[1.25rem] dark:text-[#f1f2f6]"
        >
          Book Details
        </h3>
        ...
        <div className="mt-2.5 flex flex-col items-center gap-8 overflow-auto border-t border-[#9a9aa7] pt-6 max-[43.75rem]:gap-6 max-[43.75rem]:pt-4 sm:flex-row sm:items-stretch sm:justify-center dark:border-[#2b2f3a]">
          <div className="w-[8.75rem] shrink-0 sm:w-[10rem]">
            {book.image ? (
              <img
                src={book.image}
                alt={book.name}
                className="h-auto w-full rounded-[0.375rem] shadow-[0_0.625rem_1.25rem_rgba(10,10,35,0.15)]"
              />
            ) : (
              <div className="flex h-[12.5rem] w-full items-center justify-center rounded-[0.375rem] bg-[#f1f1f6] text-[0.75rem] font-semibold text-[#6f6f76] sm:h-[15rem] dark:bg-[#26272d] dark:text-[#5a5a6a]">
                No Image
              </div>
            )}
            ...
            <button
              type="button"
              className="mt-5 w-full cursor-pointer rounded-[0.375rem] border-none bg-[#0a0b2b] px-4 py-2.5 text-center text-[0.8125rem] font-bold uppercase tracking-wider text-white hover:bg-[#14145a] max-[37.5rem]:mt-4 dark:border dark:border-[#3a3f4c] dark:bg-[#0f1116] dark:text-[#e3e6f0] dark:hover:bg-[#e3e6f0] dark:hover:text-[#0f1116]"
          ...
          <div className="flex w-full min-w-[18.75rem] grow flex-col">
