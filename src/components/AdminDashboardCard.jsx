            ...
            >
              <div className="bg-[#C7C7C7]/48 relative flex h-full flex-1 items-center gap-2.5 rounded-lg p-1.5 px-1.5 py-1 dark:bg-[#929292]">
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-md p-1.5">
                  <ShieldCheck className="h-full w-full text-[#0a0f33] dark:text-black" />
                </div>
                <div className="h-[90%] w-0.5 bg-[#0a0f33] dark:bg-[#121317]"></div>
                <div className="flex flex-1 flex-col justify-between overflow-hidden truncate whitespace-nowrap">
                  <p className="text-[16px] font-semibold text-[#0a0f33] dark:text-black">
                    {admin.name}
                  </p>
                  <p className="text-[13px] text-[#6f7390] max-[1540px]:text-[9px] dark:text-black">
                    Admin ID: {admin.adminId}
                  </p>
                  <div className="absolute bottom-0.5 right-1.5 mt-1 flex items-center justify-end gap-1">
                    <span
                      className={`h-1.5 w-1.5 rounded-full ${admin.isOnline ? "bg-[#0a0f33] dark:bg-black" : "bg-gray-400"}`}
                    ></span>
                    <span
                      className={`text-[9px] font-medium text-[#0a0f33] dark:text-black`}
                    >
              ...
              <RefreshCw
                ...
                className={`h-13 mr-1.5 w-11 cursor-pointer px-1.5 text-[#0a0f33] dark:text-black ${loadingAdmins[admin.id] ? "animate-spin" : ""}`}
              />
            </li>
          ))
        ) : (
