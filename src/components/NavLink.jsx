  ...
  return (
    <button
      ...
      className={`h-15 w-full my-1 transition-all duration-300 flex items-center ${
        isExpanded ? 'px-12 gap-2' : 'px-[39px] gap-0'
      } ${
        active
          ? 'bg-white text-[#0a0f33] rounded-l-[30px] font-medium'
          : 'text-[#b5b8d1] hover:bg-white/10'
      } max-[1080px]:gap-2 max-[1080px]:px-12`}
    >
      <span className="flex items-center" style={{ width: 22, height: 22 }}>
        {typeof icon === 'string' ? (
      ...
      <span className={`transition-all duration-300 ${isExpanded ? 'max-w-full opacity-100 ml-2' : 'max-w-0 opacity-0  ml-0'} max-[1080px]:max-w-full max-[1080px]:opacity-100 max-[1080px]:ml-2`}>{text}</span>
    </button>
  );
};
