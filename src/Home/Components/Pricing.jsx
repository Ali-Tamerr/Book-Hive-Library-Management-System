...
const PricingCard = ({ plan, setIsLoginOpen }) => {
  return (
    <article
      ...
      } cursor-default border-transparent hover:border-[var(--title-color)] dark:hover:!border-[var(--title-color)]`}
      role="listitem"
      aria-labelledby={plan.id}
    >
      <div
        id={plan.id}
        className={`cursor-default font-[family-name:var(--body-font)] text-[2.625rem] font-extrabold uppercase tracking-wider text-[var(--title-color)] max-[42.5rem]:text-[2rem] dark:text-[var(--title-color)]`}
      >
        {plan.title}
      </div>
      <div
        className={`price mb-7 cursor-default font-[family-name:Inter,sans-serif] text-[2.375rem] font-normal text-[var(--title-color)] max-[42.5rem]:mb-5 max-[42.5rem]:text-[1.875rem] dark:!text-[var(--title-color)]`}
      >
      ...
      <div className="features mb-8 mt-3 flex-1 text-left max-[42.5rem]:mb-6 max-[42.5rem]:mt-2">
        {[
          ...
          <div
            key={index}
            className="my-5 flex cursor-default items-start gap-3 font-[family-name:var(--second-font)] text-[1.1875rem] font-normal text-[var(--text-color)] max-[42.5rem]:my-3 max-[42.5rem]:text-[1rem] dark:!text-[var(--text-color)]"
          >
            <span className="mt-1 inline-block flex-none cursor-default text-[1.5rem] leading-none text-[var(--title-color)] max-[42.5rem]:text-[1.25rem] dark:!text-[var(--title-color)]">
              &#10003;
            </span>
            {feature}
          </div>
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-[105.5rem] text-center">
        <h1
          id="pricing-heading"
          className="mb-10 text-center font-[family-name:var(--body-font)] text-[3.875rem] font-extrabold tracking-wide text-[var(--title-color)] dark:!text-[var(--title-color)] max-[42.5rem]:mb-6 max-[42.5rem]:text-[2.625rem] max-[32.5rem]:text-[2.125rem]"
        >
          Choose The best Plan
        </h1>
        <p className="subheading mb-12 font-[family-name:var(--second-font)] text-[1.3125rem] font-medium text-[var(--text-color)] max-[42.5rem]:mb-8 max-[42.5rem]:text-[1rem] dark:text-[var(--text-color)]">
        ...
        <div className="plans dark:!shadow-[-0.3125rem_0.3125rem_0.3125rem_rgba(255,255,255,0.4)] flex flex-col items-stretch gap-10 rounded-[1.125rem] border border-[rgba(10,11,43,0.08)] bg-[var(--card-bg)] px-10 py-10 shadow-[-0.3125rem_0.3125rem_0.3125rem_rgba(0,0,0,0.4)] max-[62.5rem]:p-6 max-[42.5rem]:gap-6 max-[42.5rem]:p-4 xl:flex-row dark:!border-none dark:!bg-transparent">
          {isLoading ? (
            <div className="w-full py-20 text-center text-xl text-[var(--text-color)] dark:text-[var(--text-color)]">
              Loading plans...
            </div>
