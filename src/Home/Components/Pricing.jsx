...
const PricingCard = ({ plan, setIsLoginOpen }) => {
  ...
  return (
    <article
      className={`plan group flex w-full transform-gpu flex-col justify-start rounded-[18px] border-2 px-10 pb-[20px] pt-[60px] text-[var(--accent)] transition-all duration-[300ms] ease-in-out hover:-translate-y-3 hover:shadow-[0_16px_34px_rgba(0,0,0,0.2)] max-[680px]:min-h-[380px] dark:hover:shadow-[0_20px_36px_rgba(255,255,255,0.2)] ${
        plan.isPopular
    ...
    >
      <div
        id={plan.id}
        className={`cursor-default font-[family-name:var(--body-font)] text-[55px] font-extrabold uppercase tracking-wider text-[var(--accent)]`}
      >
        {plan.title}
      </div>
      <div
        className={`price mb-10 cursor-default font-[family-name:Inter,sans-serif] text-[50px] font-normal text-[var(--accent)] dark:!text-[#D7D7D7]`}
      >
      ...
      <div className="features mb-12 mt-4 flex-1 text-left">
        {[
          ...
          <div
            key={index}
            className="my-8 flex cursor-default items-start gap-3 font-[family-name:var(--second-font)] text-[23px] font-normal text-[#121317] dark:!text-[#cfd1d7]"
          >
            <span className="mt-1 inline-block flex-none cursor-default text-[28px] leading-none text-[var(--accent)] dark:!text-[#e3e4e8]">
              &#10003;
            </span>
            {feature}
          </div>
      ...
      <HomeButton
        className="mx-auto mt-4 px-20 py-3"
        href="#"
